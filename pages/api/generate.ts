...
export default async function handler(
  ...
  try {
    ...
    for (let i = 0; i < 6; i += 2) {
      ...
      const sectionPrompt = `아래는 블로그 리뷰 목차 일부이다: ${sectionTitles}\n위 목차에 해당하는 본문을 실제 블로그 글처럼 자연스럽게 이어지게 작성해줘.\n각 소제목(목차)마다 이모지, 장소명 또는 주요 키워드, 감성적 디테일을 포함하고, diary style, soliloquy, '~다'로 끝나게, 존댓말 금지, 마크다운 금지, 각 소제목은 실제 블로그 소제목처럼 자연스럽게 본문에 포함해줘.`;
