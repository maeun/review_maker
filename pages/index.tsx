...
import { ReviewTypeOptions } from "../components/ReviewTypeSelector";
...
const ReviewTypeSelector = dynamic(
  () => import("../components/ReviewTypeSelector"),
  {
    loading: () => <Box h="32" bg="gray.50" borderRadius="xl" />,
    ssr: false,
  }
);

const AdBanner = dynamic(() => import("../components/AdBanner"), {
  loading: () => <Box h="20" bg="gray.100" borderRadius="md" />,
  ssr: false,
});
...
export default function Home() {
  ...
  const handleSubmit = async () => {
    ...
    // 새로운 요청 ID 생성
    const newRequestId = `req_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="네이버 플레이스 리뷰를 분석하여 방문자 리뷰와 블로그 리뷰를 자동으로 생성합니다. 무료 서비스로 지금 바로 시작하세요."
        />
      </Head>
      <Box minH="100vh" bgGradient={bgGradient}>
        <Container maxW="6xl" py={{ base: 4, md: 8 }} px={{ base: 4, md: 6 }}>
          <VStack spacing={{ base: 6, md: 8 }}>
                ...
                <FormControl>
                  ...
                  {impressionFeedback && (
                    <Text
                      fontSize="sm"
                      color={
                        impressionFeedback.includes("반영됩니다")
                          ? "green.600"
                          : "orange.600"
                      }
                      mt={2}
            ...
            {/* 하단 광고 */}
            <AdBanner dataAdSlot="0987654321" dataAdFormat="auto" />
