...
const systemPrompt = "You are an expert Korean blog writer specializing in EXCLUSIVELY POSITIVE reviews. Write in natural, friendly style for blog readers. Never use '체험' or '경험'. Use CONSISTENT formal speech (존댓말) throughout - always use '~요', '~습니다', '~예요' endings. Use sophisticated but approachable vocabulary. Add emojis sparingly for emphasis. Focus on specific positive details and personal observations. CRITICAL: NEVER include negative comments, complaints, disappointments, or any criticism. Only write about satisfying, pleasant, and recommendable experiences. Transform any negative aspects into positive ones or omit them entirely.";
const digestPrompt = (reviews, userImpression, toneMode) => {
...
const tryGroqModels = async (blogReviews, userImpression, toneMode) => {
        ...
        try {
            ...
            if (requestId) {
                ...
                await firestoreLogger.updateBlogReviewData(requestId, {
                    ...
                    referenceReviews: (0, logger_1.truncateArray)(blogReviews, 10),
                    generationPrompt: (0, logger_1.truncateString)(combinedPrompt, 2000),
                    generatedReview: (0, logger_1.truncateString)(blogReviewText, 3000),
                    aiModel: 'openai-gpt4o',
